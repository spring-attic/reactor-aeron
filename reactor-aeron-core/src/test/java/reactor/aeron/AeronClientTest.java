...
class AeronClientTest extends BaseAeronTest {
  ...
  public void testClientReceivesDataFromServer() {
    createServer(
        connection ->
            connection
                .outbound()
                .send(Flux.fromStream(Stream.of("hello1", "2", "3")).log("server"))
  ...
  public void testClientReceivesLongDataFromServer() {
    ...
    createServer(
        connection ->
            connection
                .outbound()
                .send(Flux.fromStream(Stream.of(str, str, str)).log("server"))
                .then(connection.onDispose()));
  ...
  public void testTwoClientsReceiveDataFromServer() {
    createServer(
        connection ->
            connection
                .outbound()
                .send(Flux.fromStream(Stream.of("1", "2", "3")).log("server"))
                .then(connection.onDispose()));
  ...
  public void testClientsReceiveDataFromServer200000() {
    ...
    createServer(connection -> connection.outbound().send(payloads).then(connection.onDispose()));
  ...
  public void testRequestResponse200000() {
    ...
    connection1.outbound().send(Flux.range(0, count).map(String::valueOf)).then().subscribe();
  ...
  public void testRequestResponse200000MonoJust() {
    ...
    Flux.range(0, count)
        .flatMap(
            i -> connection1.outbound().send(Flux.fromStream(Stream.of("client_send:" + i))).then())
        .then()
        .subscribe();
  ...
  public void testTwoClientsRequestResponse200000() {
    ...
    createConnection(
        connection -> {
          ...
          Flux.range(0, count)
              .flatMap(i -> connection.outbound().send(Mono.just("client-1 send:" + i)).then())
              .then()
              .subscribe();
    ...
    createConnection(
        connection -> {
          ...
          Flux.range(0, count)
              .flatMap(i -> connection.outbound().send(Mono.just("client-2 send:" + i)).then())
              .then()
              .subscribe();
          return connection.onDispose();
        });
  ...
  public void testTwoClientsRequestResponseWithDelaySubscriptionToInbound200000() {
    ...
    AeronConnection connection1 = createConnection();
    Flux.range(0, count)
        .flatMap(i -> connection1.outbound().send(Mono.just("client-1 send:" + i)).then())
        .then()
        .subscribe();

    AeronConnection connection2 = createConnection();
    Flux.range(0, count)
        .flatMap(i -> connection2.outbound().send(Mono.just("client-2 send:" + i)).then())
        .then()
        .subscribe();
    ...
    createConnection(
        connection -> {
          for (int i = 0; i < streams; i++) {
            ...
            connection
                .outbound()
                .send(Flux.range(start, count).map(String::valueOf))
                .then()
  ...
  public void testCustomClientInboundSubscriber() {
    ...
    createServer(
        connection -> {
          connection
              .outbound()
              .send(Flux.range(0, overall).map(i -> "server send:" + i))
              .then()
              .subscribe();
          return connection.onDispose();
        });
  ...
  public void testCustomClientInboundSubscriberWithLongMessage() {
    ...
    createServer(
        connection -> {
          connection.outbound().send(Flux.range(0, overall).map(i -> i + msg)).then().subscribe();
          return connection.onDispose();
        });
