...
do
    ...
    ssh -oStrictHostKeyChecking=no -i $CERT_PATH ubuntu@$addr 'mkdir -p /tmp/reactor-aeron/scripts/'
    ssh -oStrictHostKeyChecking=no -i $CERT_PATH ubuntu@$addr 'mkdir -p /tmp/reactor-aeron/target/lib'
