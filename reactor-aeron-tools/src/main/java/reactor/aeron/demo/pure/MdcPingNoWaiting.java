...
public class MdcPingNoWaiting {
  ...
  public static void main(final String[] args) throws Exception {
    ...
    final Aeron.Context ctx =
        new Aeron.Context().availableImageHandler(MdcPingNoWaiting::availablePongImageHandler);
    ...
    try (Aeron aeron = Aeron.connect(ctx);
      ...
      for (int i = 0; i < WARMUP_NUMBER_OF_ITERATIONS; i++) {
        roundTripMessages(
            fragmentHandler, publication, subscription, WARMUP_NUMBER_OF_MESSAGES, false);
      }
  ...
  private static Disposable startReport(boolean warmup) {
    ...
    return Flux.interval(Duration.ofSeconds(Configurations.REPORT_INTERVAL))
        .doOnNext(MdcPingNoWaiting::report)
        .doFinally(MdcPingNoWaiting::report)
        .subscribe();
  }
